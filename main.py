...
try:
    ...
    if enviro.low_disk_space():
        ...
        if enviro.config.destination:
            ...
            enviro.logging.info(
                f"> {enviro.cached_upload_count()} cache file(s) need uploading"
            )
    ...
    enviro.logging.debug(
        f"> {filesystem_stats[3]} blocks free out of {filesystem_stats[2]}"
    )
    ...
    if enviro.config.enable_battery_voltage and not enviro.vbus_present:
        ...
        enviro.logging.debug(
            f"> battery voltage: {reading['battery_voltage']}, percent: {reading['battery_percent']}"
        )
    ...
    if enviro.config.destination:
        ...
        if enviro.is_upload_needed():
            ...
            enviro.logging.info(
                f"> {enviro.cached_upload_count()} cache file(s) not being uploaded. Waiting until there are {enviro.config.upload_frequency} file(s)"
            )
    else:
