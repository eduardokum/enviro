...
def provision_step_1_nickname(request):
    if request.method == "POST":
        ...
        return render_template(
            "enviro/html/provision-step-1-nickname.html", board=model
        )
...
def provision_step_3_logging(request):
    if request.method == "POST":
        ...
        config.upload_frequency = (
            int(request.form["upload_frequency"])
            if request.form["upload_frequency"]
            else None
        )
        write_config()
...
def provision_step_4_destination(request):
    if request.method == "POST":
        ...
        config.hass_discovery = hass_discovery_value.lower() in (
            "1",
            "true",
            "on",
            "yes",
        ...
        return render_template(
            "enviro/html/provision-step-4-destination.html", board=model
        )
...
def provision_step_grow_sensors(request):
    if request.method == "POST":
        ...
        return render_template(
            "enviro/html/provision-step-grow-sensors.html", board=model
        )
