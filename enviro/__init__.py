...
def get_qwst_modules():
    ...
    if I2C_ADDR_LTR390 in i2c_devices:  # LTR390
        try:
            ...
            modules.append(
                {"name": "LTR390", "include": ltr390, "address": I2C_ADDR_LTR390}
            )
        except RuntimeError:
            pass
    ...
    if I2C_ADDR_SCD41 in i2c_devices:  # SCD41
        try:
            ...
            modules.append(
                {"name": "SCD41", "include": scd41, "address": I2C_ADDR_SCD41}
            )
        except RuntimeError:
            pass
...
def activity_led_callback(t):
    ...
    brightness = (
        math.sin(time.ticks_ms() * math.pi * 2 / (1000 / activity_led_pulse_speed_hz))
        * 40
    ) + 60
    value = int(pow(brightness / 100.0, 2.8) * 65535.0 + 0.5)
    activity_led_pwm.duty_u16(value)
...
def pulse_activity_led(speed_hz=1):
    ...
    activity_led_timer.init(
        period=50, mode=Timer.PERIODIC, callback=activity_led_callback
    )
...
def reconnect_wifi(ssid, password, country, hostname=None):
    ...
    def dump_status():
        status = wlan.status()
        logging.info(
            f"> active: {1 if wlan.active() else 0}, status: {status} ({status_names[status]})"
        )
        return status
...
def connect_to_wifi():
    ...
    try:
        ...
        elapsed_ms = reconnect_wifi(
            config.wifi_ssid, config.wifi_password, config.wifi_country
        )
...
def is_clock_set():
    ...
    if helpers.file_exists("sync_time.txt"):
        ...
        ):  # there's the rare chance of having a newer sync time than what the RTC reports
            try:
                if seconds_since_sync < (config.resync_frequency * 60 * 60):
                    return True

                logging.info(
                    f"  - rtc has not been synched for {config.resync_frequency} hour(s)"
                )
...
def get_qwst_modules_readings():
    ...
    for module in modules:
        ...
        module_readings = module_readings | module["include"].get_readings(
            i2c, module["address"]
        )
...
def upload_readings():
    ...
    try:
        ...
        if (
            secondary_destination in valid_secondary_destinations
            and secondary_destination != destination
        ):
            exec(f"import enviro.destinations.{secondary_destination}")
            secondary_destination_module = sys.modules[
                f"enviro.destinations.{secondary_destination}"
            ]
            ...
            try:
                with open(f"uploads/{cache_file[0]}", "r") as upload_file:
                    ...
                    if status == UPLOAD_SUCCESS:
                        logging.info(
                            f" - Primary destination upload success for {filename}"
                        )
                        ...
                        logging.info(
                            f"  - cannot upload '{cache_file[0]}' - rate limited"
                        )
                        sleep(1)
                        ...
                        logging.info(
                            f"  - cannot upload '{cache_file[0]}' - rtc has become out of sync"
                        )
                        sleep(1)
                    elif status == UPLOAD_SKIP_FILE:
                        logging.error(
                            f"  ! cannot upload '{cache_file[0]}' to {destination}. Skipping file"
                        )
                        warn_led(WARN_LED_BLINK)
                        continue
                    else:
                        logging.error(
                            f"  ! failed to upload '{cache_file[0]}' to {destination}"
                        )
                        return False
                    ...
                    if (
                        ...
                        secondary_status = secondary_destination_module.upload_reading(
                            json
                        )
                        if secondary_status == UPLOAD_SUCCESS:
                            logging.info(
                                f"  - Secondary destination upload success for {filename}"
                            )
                ...
                logging.error(
                    f"  ! skipping '{cache_file[0]}' as it is missing data. It was likely created by an older version of the enviro firmware"
                )
...
def sleep(time_override=None):
    ...
    if time_override is not None:
        ...
        if second > 55:
            minute += 1
        minute = (
            math.floor(minute / config.reading_frequency) * config.reading_frequency
        )
    ...
    logging.debug(
        "  - on usb power (so can't shutdown). Halt and wait for alarm or user reset instead"
    )
    board = get_board()
